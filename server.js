...
app.get('/api/reporte-campo/:os', async (req, res) => { try { res.json((await pool.query('SELECT * FROM reporte_campo WHERE os=$1', [req.params.os])).rows.map(r => ({ ...r, ...r.datos }))); } catch (e) { fail(res, e, 'REPORTE_GET'); } });
app.post('/api/reporte-campo', async (req, res) => { const r = req.body; try { const saved = await tx(async c => { const os = await osRow(c, r.os); return (await c.query(`INSERT INTO reporte_campo(id_os,os,matriz,submatriz,parametro,punto,reporte,datos) VALUES($1,$2,$3,$4,$5,$6,$7,$8) ON CONFLICT(id_os,submatriz,parametro,punto) DO UPDATE SET reporte=EXCLUDED.reporte,datos=EXCLUDED.datos RETURNING *`, [os.id_os, r.os, r.matriz, r.submatriz, r.parametro, r.punto, r.reporte, js(r)])).rows[0]; }); res.status(201).json(saved); } catch (e) { fail(res, e, 'REPORTE_UPSERT'); } });
